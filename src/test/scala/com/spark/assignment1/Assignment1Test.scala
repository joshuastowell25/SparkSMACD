...
class Assignment1Test extends AnyFunSuite with Matchers with BeforeAndAfterEach {
  ...
  test("Find the longest trip (duration)") {
    Assignment1Driver.problem1(tripDataRdd) must equal(17270400)  //PASSING
  }
  ...
  test("All trips starting at the 'San Antonio Shopping Center' station") {
    Assignment1Driver.problem2(tripDataRdd) must equal(1069) //there are 1069 trips that start there.  //PASSING
  }
  ...
  test("List out all subscriber types") {
    Assignment1Driver.problem3(tripDataRdd).toSet must equal(Set("Customer", "Subscriber"))  //PASSING
  }
  ...
  test("Busiest zipcode") {
    Assignment1Driver.problem4(tripDataRdd) must equal("94107")  //PASSING
  }
  ...
  test("Trips that went overnight") {
    Assignment1Driver.problem5(tripDataRdd) must equal(920)  //PASSING
  }
  ...
  test("Get the dataset count") {
    Assignment1Driver.problem6(tripDataRdd) must equal(354152) //PASSING
  }
  ...
  test("Get the percentage of trips that went overnight") {
    Assignment1Driver.problem7(tripDataRdd) must be(0.0025 +- .0003) //PASSING
  }
  ...
  test("Double the duration of each trip") {
    Assignment1Driver.problem8(tripDataRdd) must equal (7.40909118E8) //PASSING
  }
  ...
  test("Coordinates of trip id 913401") {
    Assignment1Driver.problem9(tripDataRdd, stationDataRdd) must equal ((37.781039,-122.411748)) //PASSING
  }

  /**
   * Find the duration of all trips by starting at each station.
   * To complete this you will need to join the Station and Trip RDDs. <-----Not really true. See commented code in implementation.
   *
  ...
  test("Duration by station") {
    val result = Assignment1Driver.problem10(tripDataRdd, stationDataRdd).toSeq
  ...
  test("Select the 'trip_id' column") {
    Assignment1Driver.dfProblem11(tripDataDF).schema.length must equal (1) //PASSING
  }
  ...
  test("Count of all trips starting at 'Harry Bridges Plaza (Ferry Building)'") {
    Assignment1Driver.dfProblem12(tripDataDF).count() must equal (17255) //PASSING
  }
  ...
  test("Sum the duration of all trips") {
    Assignment1Driver.dfProblem13(tripDataDF) must equal (370454559) //TODO
  }
}
